...
exportObj.translations.English = {
  ...
  ui: {
    ...
    collectionContentShipsAndPilots: function(translator, data) { // data[0] is ships, data[1] is pilots
...
<p><button class="btn btn-success podcast" onclick="window.open('http://offmeta.club');">Off-Meta Podcast</button> <button class="btn btn-primary paypal" onclick="window.open('https://paypal.me/raithos');">Donate</button> <button class="btn btn-primary patreon" onclick="window.open('https://www.patreon.com/raithos');">Patreon</button> </p>`,
    ...
    adds: function(translator, data) { // data will most likely be a string of some symbols, but you never know
      return `Adds: ${translator('ui', data)}`;
    },
    removes: function(translator, data) { // data will most likely be a string of some symbols, but you never know
...
// in this file the display_name is often omitted, if it is identical to the name. I am too lazy to add those manually (though running the translation script would automatically do so). As this will pop up with every new card, I add those tags here
  ...
  for (name in damage_translations) {
    data = damage_translations[name];
    if (data.display_name == null) {
      data['display_name'] = name;
    }
  }
...
//# sourceMappingURL=translation-en.js.map
